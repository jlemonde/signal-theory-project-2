...
for k = 1:length(key)
    for l = 0:3
        if k-l >= 1
            rKey(k) = rKey(k) + h(l+1)*key(k-l) + normrnd(0, 0.1);
        end
    end
end
...
for L = 0:maxL
    ...
    MSEArray(L+1) = SE / length(errCheck);
    
end
plot(0:maxL, MSEArray);
xlabel(["Mean square error between the recovered signal and the original key, ",
    "on another image generated by us so that we know the original key."]);
return;
